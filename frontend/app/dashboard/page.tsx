...
const FEATURES = [
  ...
  { href: "/dashboard/chat",      icon: "💬", title: "Ask Sahayak",       desc: "Chat with your AI study friend — text or file-based doubts, anytime.", color: "#c084fc", plan: "basic" },
];
...
export default function DashboardHome() {
  ...
  useEffect(() => {
    ...
    if (!parsed.onboarded) { router.push("/onboarding"); return; }
    setUser(parsed);
  }, []);
  ...
  return (
    <div>
      ...
      <div className="welcome">
        ...
        <p>{user.category} {user.level} {user.college ? `· ${user.college}` : ""}</p>
      </div>
      ...
      {user.plan !== "pro" && (
        <div className="upgrade-banner">
          ...
          <Link href="/profile?tab=plan" className="upgrade-btn">Upgrade →</Link>
        </div>
      )}
...
.grid {
  ...
  margin-bottom: 24px;
        ...
        .upgrade-banner { background:linear-gradient(135deg,rgba(74,222,128,0.1),rgba(34,197,94,0.05)); border:1px solid rgba(74,222,128,0.2); border-radius:16px; padding:20px 24px; display:flex; align-items:center; justify-content:space-between; gap:16px; flex-wrap:wrap; }
        .upgrade-banner strong { color:#4ade80; }
        .upgrade-banner span { color:rgba(255,255,255,0.5); font-size:14px; }
        .upgrade-btn { background:linear-gradient(135deg,#4ade80,#22c55e); color:#0f2027; font-weight:700; padding:10px 20px; border-radius:12px; text-decoration:none; font-size:14px; white-space:nowrap; }
